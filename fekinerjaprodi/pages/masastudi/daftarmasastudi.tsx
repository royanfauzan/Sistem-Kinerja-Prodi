...
export default function daftarprestasi() {
  ...
  const tambahipk = () => {
    MySwal.fire({
      ...
      if (result.value) {
        // <-- if confirmed
        router.push(`/masastudi/inputmasastudi`);
      }
    });
  };
  ...
  const exportKjs = () => {
    MySwal.fire({
      ...
      if (result.value) {
        // <-- if confirmed
        router.push(`/masastudi/exportmasastudi/export_masastudi`);
      }
    });
  };
  ...
  const deleteipk = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios({
          method: "post",
          url: `http://127.0.0.1:8000/api/ipk_delete/${id}`,
        })
          .then(function (response) {
            router.reload();
          })
  ...
  const searchdata = async (e) => {
    if (e.target.value == "") {
      const req = await axios.get(`http://127.0.0.1:8000/api/masastudi/`);
      const res = await req.data.all_masastudi;
      setprofilDosen(res);
    } else {
      const req = await axios.get(
        `http://127.0.0.1:8000/api/cari_masastudi/${e.target.value}`
      );
      const res = await req.data.searchmasastudi;
      setprofilDosen(res);
    }
  };
