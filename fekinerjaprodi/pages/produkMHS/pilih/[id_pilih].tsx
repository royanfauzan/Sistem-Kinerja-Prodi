...
export async function getServerSideProps(context) {
  ...
  const req = await axios.get(
    `http://127.0.0.1:8000/api/tampil_ProdukMHS/${context.query.id_pilih}`
  );
  const res = await req.data.all_produk;

  return {
    props: {
      produk: res, // <-- assign response
    },
  };
}
...
export default function editproduk(props) {
  ...
  const { produk } = props;
  const [dataProduk, setProduk] = useState(produk);
  ...
  const [dataProduks, setProduks] = useState([]);
  const [selectProduk, setSelectProduk] = useState(produk.mahasiswa_id);
  ...
  const handleChangeLuaran = (e) => {
    setSelectProduk(e.target.value);
  };
  ...
  const submitForm = async (event) => {
    ...
    formData.append("produk_id", selectId);

    axios({
      method: "post",
      url:
        `http://127.0.0.1:8000/api/produk_mhs/${dataProduk.id}`,
      data: formData,
      headers: {
