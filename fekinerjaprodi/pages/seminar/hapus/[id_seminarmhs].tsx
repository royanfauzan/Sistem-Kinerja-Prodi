...
export async function getServerSideProps(context) {
  ...
  const req = await axios.get(
    `http://127.0.0.1:8000/api/tampil_relasi_seminar/${context.query.id_seminarmhs}`
  );
...
export default function daftarseminar(props) {
  const router = useRouter();
  const { id_seminarmhs } = router.query;
  const { seminar } = props;

  console.log(router.query)

  const [stadmin, setStadmin] = useState(false);
  const [profilDosen, setprofilDosen] = useState(seminar);
  const [id_seminar, setid_seminar] = useState(id_seminarmhs);
  ...
  const pengambilData = async () => {
    ...
    axios({
      method: "get",
      url: `http://127.0.0.1:8000/api/tampil_relasi_seminar/${id_seminar}`,
  ...
  const deleteluaran = (id) => {
    axios({
      method: "post",
      url: `http://127.0.0.1:8000/api/deletemahasiswa_seminar/${id}`,
    })
      .then(function (response) {
        router.reload();
      })
                  ...
                  <div className="row justify-content-between mb-4">
                    <div className="col-4">
                      <td className="align-middle">
                        <Link href={`/seminar/daftarseminar/`}>
