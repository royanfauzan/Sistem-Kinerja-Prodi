...
export default function tabeltulisan() {
  const router = useRouter();
  const MySwal = withReactContent(Swal);
  const [stadmin, setStadmin] = useState(false);
  const apiurl = "http://127.0.0.1:8000/"

  const [dataTulisans, setdataTulisans] = useState([]);
  ...
  const pengambilData = async () => {
    ...
    axios({
      method: "get",
      url: "http://127.0.0.1:8000/api/search_tulisandosdsn/",
      ...
      .then(function (response) {
        ...
        setdataTulisans(datatulisans);

        console.log(datatulisans);
      })
      .catch(function (err) {
        console.log("gagal");
  ...
  useEffect(() => {
    ...
    axios({
      ...
      .then(function (response) {
        ...
        if (level_akses !== 2) {
          return router.push("/tulisandos/export/exporttulisan");
        }
  ...
  const deletetulisan = (id) => {
    MySwal.fire({
      title: "Yakin Hapus Data tulisan?",
  ...
  const searchdata = async (e) => {
    const lgToken = localStorage.getItem("token");
    if (e.target.value == "") {
      const req = await axios.get(`http://127.0.0.1:8000/api/search_tulisandosdsn/`, {
        headers: {
          Authorization: `Bearer ${lgToken}`
        }
      });
      const res = await req.data.datatulisans;
      setdataTulisans(res);
    } else {
      const req = await axios.get(
        `http://127.0.0.1:8000/api/search_tulisandosdsn/${e.target.value}`, {
          headers: {
            Authorization: `Bearer ${lgToken}`
          }
        }
                    ...
                    <div className="row justify-content-between mb-4">
                      ...
                      <div className="col-6">
                        <div className="row">
                          ...
                          <Link href={`/tulisandos/inputtulisandsn/`}>
                ...
                <div className="card-body px-0 pt-0 pb-2">
                  <div className="table-responsive p-0">
                    <table className="table align-items-center mb-0 mx-4">
                      ...
                      <tbody>
                        {dataTulisans && dataTulisans.map((tulisan) => {
                          return (
                            <tr
                              className="text-center"
                              key={`ttulisan` + tulisan.id}
                            >
                              <td className="align-middle text-sm">
                                <p className="text-xs font-weight-bold mb-0 pe-3">
                                  {tulisan.profil_dosen.NamaDosen}
                                  
                                </p>
                              </td>
                              <td className="align-middle text-sm" >
                                <p className="text-xs font-weight-bold mb-0 pe-3">
                                  {`${tulisan.judul.slice(0,40)}...`}
                                </p>
                              </td>
                              <td className="align-middle text-sm">
                                <p className="text-xs font-weight-bold mb-0 pe-3">
                                  {tulisan.ruang_lingkup}
                                </p>
                              </td>
                              <td className="align-middle text-sm">
                                <p className="text-xs font-weight-bold mb-0 pe-3">
                              ...
                              <td className="align-middle text-sm">
                                <p className="text-xs font-weight-bold mb-0 pe-3">
                                <a href={`${apiurl+tulisan.file_bukti}`}>{tulisan.file_bukti.split("/").slice(-1)[0] }</a>
                                </p>
                              </td>
                              <td className="align-middle pe-3 text-end">
                                <Link href={`/tulisandos/edit/${tulisan.id}`}>
                                ...
                                <button
                                  onClick={() => deletetulisan(tulisan.id)}
