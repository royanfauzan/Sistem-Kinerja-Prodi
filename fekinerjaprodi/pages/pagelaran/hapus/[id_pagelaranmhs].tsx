...
export async function getServerSideProps(context) {
  ...
  const req = await axios.get(
    `http://127.0.0.1:8000/api/tampil_relasi/${context.query.id_pagelaranmhs}`
  );
...
export default function daftarpagelaran(props) {
  const router = useRouter();
  const { id_pagelaranmhs } = router.query;
  const { pagelaran } = props;

  console.log(router.query)

  const [stadmin, setStadmin] = useState(false);
  const [profilDosen, setprofilDosen] = useState(pagelaran);
  const [id_pagelaran, setid_pagelaran] = useState(id_pagelaranmhs);
  ...
  const pengambilData = async () => {
    ...
    axios({
      method: "get",
      url: `http://127.0.0.1:8000/api/tampil_relasi_pagelaran/${id_pagelaran}`,
  ...
  const deleteluaran = (id) => {
    axios({
      method: "post",
      url: `http://127.0.0.1:8000/api/deletemahasiswa_pagelaran/${id}`,
    })
      .then(function (response) {
        router.reload();
      })
